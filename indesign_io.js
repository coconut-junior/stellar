...
async function getScriptPath() {
    ...
    exec(versionCmd, (error, stdout, stderr) => {
        ...
        let versionString = 'Version ' + versionNumber + '.0';
    ...
    exec(`${script}`, (error, stdout, stderr) => {
        if (error) {
            ...
            if (error.message.includes('authorize')) {
                showError(`Looks like you're missing automation permissions. Please allow Stellar to control InDesign by granting it permission in the Settings app on your Mac.`);
...
//# sourceMappingURL=indesign_io.js.map